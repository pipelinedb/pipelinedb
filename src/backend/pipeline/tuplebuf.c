...
TupleBufferInsert(TupleBuffer *buf, StreamTuple *tuple, Bitmapset *queries)
{
	...
	if (!bms_is_empty(buf->waiters))
	{
		...
		while ((i = bms_first_member(notify)) >= 0)
		{
			...
			if (queries != NULL && OidIsValid(reader->proc->group->db_oid) &&
					reader->proc->group->db_oid != slot->db_oid)
				continue;
...
TupleBufferPinNextSlot(TupleBufferReader *reader)
{
	...
	while (true)
	{
		...
		if ((!OidIsValid(reader->proc->group->db_oid) || reader->proc->group->db_oid == reader->slot->db_oid) &&
...
TupleBufferTryWait(TupleBufferReader *reader)
{
	...
	if (should_wait)
		WaitLatch(reader->proc->latch, WL_LATCH_SET | WL_POSTMASTER_DEATH, IsContQueryCombinerProcess() ? 1 : 0);
}
