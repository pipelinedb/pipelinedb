...
CREATE CONTINUOUS VIEW v AS SELECT count(DISTINCT x::int) FROM stream;
CREATE TABLE t (count int);
CREATE OR REPLACE FUNCTION tgfunc()
 RETURNS trigger AS
 $$
 BEGIN
  INSERT INTO t (count) VALUES (NEW.count);
  RETURN NEW;
 END;
 $$
 LANGUAGE plpgsql;
...
DROP TRIGGER tg ON v;
TRUNCATE CONTINUOUS VIEW v;
TRUNCATE TABLE t;

CREATE TRIGGER tg AFTER UPDATE OR INSERT ON v FOR EACH ROW WHEN (NEW.count > 5) EXECUTE PROCEDURE tgfunc();
