...
FROM stream GROUP BY key;

ACTIVATE test_min_max;
...
SELECT key, i8min, i8max, i4min, i4max, i2min, i2max FROM test_min_max;
SELECT key, omin, omax, f8min, f8max, f4min, f4max FROM test_min_max;
SELECT key, dmin, dmax, tmin, tmax, tzmin, tzmax, mmin FROM test_min_max;
SELECT key, mmax, tsmin, tsmax, tstzmin, tstzmax, intervalmin FROM test_min_max;
SELECT key, intervalmax, txtmin, txtmax, nmin, nmax, amin, amax FROM test_min_max;

DROP CONTINUOUS VIEW test_min_max;
