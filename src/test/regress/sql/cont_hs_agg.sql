CREATE FOREIGN TABLE test_hs_stream (x int, y int, z text, g int) SERVER pipelinedb;

-- rank
CREATE VIEW test_rank0 AS
  SELECT
    rank(7, 47, '47'::text) WITHIN GROUP (ORDER BY x::integer, y::integer, z::text)
  FROM test_hs_stream;

CREATE VIEW test_rank1 AS
  SELECT
    g::integer,
    rank(5, 5) WITHIN GROUP (ORDER BY g, x::integer)
  FROM test_hs_stream
GROUP BY g;

CREATE VIEW test_rank2 AS
  SELECT
    g::integer,
    rank(-10, 10) WITHIN GROUP (ORDER BY g, x::integer)
  FROM test_hs_stream
GROUP BY g;

CREATE VIEW test_rank3 AS
  SELECT
    g::integer,
    rank(2, 1000, 1000) WITHIN GROUP (ORDER BY g, x::integer, y::integer) + rank(1000000) WITHIN GROUP (ORDER BY x) AS rank_sum
  FROM test_hs_stream
GROUP BY g;

-- percent_rank
CREATE VIEW test_percent0 AS
  SELECT
    percent_rank(2) WITHIN GROUP (ORDER BY x::integer)
  FROM test_hs_stream;

CREATE VIEW test_percent1 AS
  SELECT
    g::integer,
    percent_rank('7'::text) WITHIN GROUP (ORDER BY z::text) AS p0,
    percent_rank('00'::text) WITHIN GROUP (ORDER BY z) AS p1
  FROM test_hs_stream
GROUP BY g;

CREATE VIEW test_percent2 AS
  SELECT
    percent_rank(27, -27, '27'::text) WITHIN GROUP (ORDER BY x::integer, y::integer, z::text)
  FROM test_hs_stream;

CREATE VIEW test_percent3 AS
  SELECT
    percent_rank(10.1::float8, 10.2::float8) WITHIN GROUP (ORDER BY x::float8, y::float8)
  FROM test_hs_stream;

-- cume_dist
CREATE VIEW test_cume_dist0 AS
  SELECT
    cume_dist(2 * 6) WITHIN GROUP (ORDER BY -x::integer)
  FROM test_hs_stream;

CREATE VIEW test_cume_dist1 AS
  SELECT
    g::integer,
    cume_dist(10, -10) WITHIN GROUP (ORDER BY g, x::integer) AS c0,
    cume_dist(2, -2) WITHIN GROUP (ORDER BY g, x) AS c1
  FROM test_hs_stream
GROUP BY g;

CREATE VIEW test_cume_dist2 AS
  SELECT
    cume_dist(50, -50, '50'::text) WITHIN GROUP (ORDER BY x::integer, y::integer, z::text)
  FROM test_hs_stream;

CREATE VIEW test_cume_dist3 AS
  SELECT
    cume_dist(10.1::float8, 10.2::float8) WITHIN GROUP (ORDER BY x::float8, y::float8)
  FROM test_hs_stream;

-- dense_rank
CREATE VIEW test_dense_rank0 AS
  SELECT
    dense_rank(10) WITHIN GROUP (ORDER BY x::integer),
    rank(10) WITHIN GROUP (ORDER BY x)
  FROM test_hs_stream;

CREATE VIEW test_dense_rank1 AS
  SELECT
    dense_rank(substring('30', 1, 2)) WITHIN GROUP (ORDER BY z::text)
  FROM test_hs_stream;

CREATE VIEW test_dense_rank2 AS
  SELECT
    dense_rank(30, -30, '30'::text) WITHIN GROUP (ORDER BY x::integer, y::integer, z::text),
    rank(30, -30, '30'::text) WITHIN GROUP (ORDER BY x::integer, y::integer, z::text)
  FROM test_hs_stream;
...
DROP FOREIGN TABLE test_hs_stream CASCADE;
