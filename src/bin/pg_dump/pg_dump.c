...
/*
 * is_matrel_user_index
 *
 * Returns true iff the given index is not an index created by CREATE CONTINUOUS VIEW,
 * which is indicated by it being the primary index on a matrel or an expression index
 * with a specific name.
 */
static bool
is_matrel_user_index(Archive *fout, Oid index_oid)
{
	PQExpBuffer pq = createPQExpBuffer();
	PGresult *res;
 ...
 */
static bool
is_output_stream(Archive *fout, TableInfo *ti)
{
	...
	res = ExecuteSqlQuery(fout, ps->data, PGRES_TUPLES_OK);
		...
		char *cv_namespace, char *cv_name)
{
	...
	appendPQExpBuffer(upgrade_buffer,
			"SELECT create_cv_set_next_oids_for_matrel("
	...
	appendPQExpBuffer(upgrade_buffer,
			"SELECT create_cv_set_next_oids_for_osrel("
